...
namespace CDR.Register.Admin.API.Business.Validators
{
    public class DataHolderAuthenticationValidator : AbstractValidator<DataHolderAuthenticationModel>
    {
        public DataHolderAuthenticationValidator()
        {
            ...
            // Enum Validations
            RuleFor(x => x.RegisterUType).Must(x => Enum.TryParse(x.Replace("-", string.Empty), true, out RegisterUType _)).WithErrorCode(ErrorCodes.Cds.InvalidField).WithMessage(ErrorTitles.InvalidField);
