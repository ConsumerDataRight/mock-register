...
namespace CDR.Register.IntegrationTests.IdentityServer
{
    ...
    /// </summary>   
    public class IdentityServer_Token_Tests : BaseTest
    {
        protected const string CLIENTASSERTION_ISSUER = "86ecb655-9eba-409c-9be3-59e7adf7080d";
        protected const string CLIENTASSERTION_AUDIENCE = "https://localhost:7001/idp/connect/token";
        ...
        private static string GetClientAssertion(
            string certificateFilename = CERTIFICATE_FILENAME,
            string certificatePassword = CERTIFICATE_PASSWORD,
            string issuer = CLIENTASSERTION_ISSUER,
            string audience = CLIENTASSERTION_AUDIENCE)
        {
            var tokenizer = new PrivateKeyJwt(certificateFilename, certificatePassword);
            return tokenizer.Generate(issuer, audience);
        }
        ...
        [Fact]
        public async Task TokenRequest_Valid_ShouldRespondWith_200OK_AccessToken()
        {
            ...
            using (new AssertionScope())
            {
                ...
                if (accessToken != null)
                {
                    ...
                    if (validatedToken != null)
                    {
                        ...
                        cnf.Value.Should().Contain("58D76F7A61CD726DA1C54F6898E8E69EA4C88060");
                    }
                }
            }
        }
        ...
        public async Task TokenRequest_InvalidGrantType_ShouldRespondWith_400BadRequest_ErrorResponse(string grantType)
        {
            // Arrange
            var client = GetClientWithCertificate();
        ...
        public async Task TokenRequest_InvalidClientId_ShouldRespondWith_400BadRequest_ErrorResponse(string clientid)
        {
            // Arrange
            var client = GetClientWithCertificate();
        ...
        public async Task TokenRequest_InvalidClientAssertionType_ShouldRespondWith_400BadRequest_ErrorResponse(string clientAssertionType)
        {
            // Arrange
            var client = GetClientWithCertificate();
        ...
        [InlineData("")] // blank 
        [InlineData("foo")] // invalid
        public async Task TokenRequest_InvalidClientAssertion_ShouldRespondWith_400BadRequest_ErrorResponse(string clientAssertion)
        {
            // Arrange
            var client = GetClientWithCertificate();
        ...
        public async Task TokenRequest_ValidWithInvalidClientCertificate_ShouldRespondWith_400BadRequest_ErrorResponse(string certificateFilename, string certificatePassword)
        {
            // Arrange
